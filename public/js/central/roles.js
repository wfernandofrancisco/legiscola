...
document.addEventListener('DOMContentLoaded', function () {
    (function () {
        // ── Elementos do DOM ────────────────────────────────────────────────
        const rolesTableWrapper = document.getElementById('roles-table-wrapper');
        ...
        function loadTable(url) {
            if (!rolesTableWrapper) return;

            rolesTableWrapper.style.opacity = '0.5';
            ...
            fetch(url, {
                ...
                .then(html => {
                    rolesTableWrapper.innerHTML = html;
                    rolesTableWrapper.style.opacity = '1';
        ...
        document.addEventListener('click', function (e) {
            const link = e.target.closest('#roles-table-wrapper a[href]');
            ...
            try {
                ...
                if (sortByParam) {
                    const sortByInput = roleFilterForm?.querySelector('[name="sort_by"]');
                    if (sortByInput) sortByInput.value = sortByParam;
                }
                if (sortDirParam) {
                    const sortDirInput = roleFilterForm?.querySelector('[name="sort_dir"]');
