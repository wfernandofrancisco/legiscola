...
@section('title', 'Login Central — ' . config('app.name'))
    ...
    <div class="relative z-10 mx-auto grid min-h-[calc(100vh-5.5rem)] max-w-6xl items-center gap-10 px-5 pb-12 pt-4 sm:px-8 lg:grid-cols-12 lg:gap-8">
        <div class="lg:col-span-7">
            ...
            <p class="auth-in auth-d3 mt-5 max-w-md text-lg leading-relaxed text-slate-600">
                Painel da operação central: câmaras, usuários e a plataforma inteira —
                sem misturar com o login da câmara ou da direção regional.
            </p>
            ...
            <div class="auth-in auth-d4 relative mt-10 hidden h-56 sm:block lg:h-64">
                <div class="auth-float absolute left-0 top-2 w-48 rounded-2xl bg-white/85 p-4 shadow-xl shadow-indigo-300/40 ring-1 ring-white backdrop-blur">
                    <p class="text-[10px] font-bold uppercase tracking-wider text-indigo-600">Rede</p>
                    <p class="font-display mt-1 text-lg font-bold text-slate-900">Câmaras ativas</p>
                ...
                <div class="auth-float-slow absolute left-40 top-16 w-52 rounded-2xl bg-indigo-700 p-4 text-white shadow-xl shadow-indigo-500/40" style="animation-delay:-1.5s">
                    <p class="text-[10px] font-bold uppercase tracking-wider text-indigo-200">Plataforma</p>
                    <p class="font-display mt-1 text-lg font-bold">Contas e módulos</p>
                    <p class="mt-2 text-xs text-indigo-100">Você opera · a câmara usa</p>
                </div>
                <div class="auth-float absolute left-16 top-36 w-52 rounded-2xl bg-amber-300 p-4 shadow-xl shadow-amber-400/50" style="animation-delay:-3s">
                    <p class="text-[10px] font-bold uppercase tracking-wider text-amber-900/70">Acesso</p>
                    <p class="font-display mt-1 text-lg font-bold text-slate-900">Somente central</p>
                </div>
                <div class="auth-float-slow absolute right-4 top-0 hidden w-40 rounded-full bg-white/80 p-3 text-center shadow-lg ring-1 ring-white xl:block" style="animation-delay:-2s">
                    <p class="font-serif text-sm italic text-slate-700">rede · operação · controle</p>
                </div>
            </div>
        </div>
        ...
        <div class="auth-in auth-d5 lg:col-span-5">
            <div class="relative rounded-[1.75rem] bg-white/80 p-6 shadow-2xl shadow-indigo-400/20 ring-1 ring-white backdrop-blur-xl sm:p-8">
                ...
                <h2 class="font-display text-2xl font-extrabold tracking-tight text-slate-900">Login central</h2>
                <p class="mt-1 text-sm text-slate-500">Acesso exclusivo do operador da plataforma.</p>
                ...
                <form action="{{ route('central.login.store') }}" method="POST" class="mt-6 space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <input type="email" id="email" name="email" value="{{ old('email') }}"
                               ...
                               placeholder="central@{{ config('app.domain', 'legiscola.test') }}" required autofocus />
                        @error('email')
                ...
                <p class="mt-6 text-center text-[11px] leading-relaxed text-slate-500">
                    ...
                    @if (Route::has('diretor.login'))
                        <span class="mx-1">·</span>
                        <a href="{{ route('diretor.login') }}" class="font-semibold text-indigo-700 hover:text-indigo-900">Login do diretor</a>
                    @endif
                </p>
            </div>
        </div>
