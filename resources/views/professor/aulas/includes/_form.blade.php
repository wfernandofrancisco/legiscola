<form method="POST" enctype="multipart/form-data"
    action="{{ $action === 'edit' ? route('professor.aulas.update', $classLesson) : route('professor.aulas.store') }}"
    ...
    @php
        $selectedCourseClassId = old('course_class_id', $classLesson?->course_class_id ?? ($prefillCourseClassId ?? ''));
        $selectedCourseClassName = old(
            'course_class_search',
            $classLesson?->courseClass?->name ?? ($prefillCourseClassName ?? '')
        );
    @endphp
    ...
    @push('scripts')
        <script>
            (function() {
                ...
                var timer;
                var searchUrl = '{{ route('professor.aulas.turmas.search') }}';
