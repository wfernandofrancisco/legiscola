...
<div class="mb-8 space-y-6 rounded-xl border border-slate-200/80 bg-slate-50/30 p-4 dark:border-slate-700 dark:bg-slate-900/20 sm:p-6">
    ...
    <div class="rounded-xl border border-indigo-200 bg-indigo-50/50 p-5 dark:border-indigo-900 dark:bg-indigo-950/25">
        ...
        <form method="GET" action="{{ route('professor.turmas.ficha-presenca', $turma) }}" class="mt-4 flex flex-wrap items-end gap-3">
            ...
            <div class="inline-flex min-w-full flex-wrap gap-2 rounded-xl border border-gray-200 bg-gray-50 p-2 dark:border-gray-600 dark:bg-gray-900/50">
                @foreach ($lessonSheetLessons as $lessonRow)
                    ...
                    <div class="inline-flex items-center gap-1 rounded-lg border px-2 py-1
                        ...
                        <a href="{{ route('professor.turmas.ficha-presenca', [
                            'turma' => $turma,
                        ...
                        @if ($canManageTab)
                            <form method="POST" action="{{ route('professor.turmas.ficha-presenca.destroy', $turma) }}"
                                class="inline"
    ...
    @if ($lessonActiveLesson)
        <div class="flex flex-wrap items-end justify-between gap-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('professor.turmas.ficha-presenca.print', ['turma' => $turma, 'lesson' => $lessonActiveLesson->id, 'mode' => 'blank']) }}" target="_blank"
                ...
                <a href="{{ route('professor.turmas.ficha-presenca.print', ['turma' => $turma, 'lesson' => $lessonActiveLesson->id, 'mode' => 'filled']) }}" target="_blank"
    ...
    <form method="POST" action="{{ route('professor.turmas.ficha-presenca.store', $turma) }}">
        @csrf
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-10 text-center text-gray-500">
                                Nenhum aluno com matrícula ativa nesta turma (inscrito/cursando/concluído/baixa_presença).
                            </td>
                        </tr>
                    @endforelse
                </tbody>
