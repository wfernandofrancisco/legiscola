...
<div class="mb-6 rounded-xl border border-emerald-200 bg-emerald-50/70 p-4 dark:border-emerald-900 dark:bg-emerald-950/25">
    ...
    <form id="quick-lesson-form" method="POST"
        action="{{ route('professor.turmas.ficha-presenca.aula-rapida.store', $turma) }}"
        ...
        <div class="sm:col-span-2 lg:col-span-2">
            ...
            <input id="quick-lesson-title" name="title" type="text" required maxlength="255"
                value="{{ old('title', 'Aula') }}"
        ...
        <div>
            ...
            <input id="quick-lesson-start" name="start_time" type="time" required step="60"
                value="{{ old('start_time') ? \App\Support\ClockTime::toHi(old('start_time')) : ($defaultScheduleStart ?? '19:00') }}"
                class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white" />
            @error('start_time')
                <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <input id="quick-lesson-end" name="end_time" type="time" required step="60"
                value="{{ old('end_time') ? \App\Support\ClockTime::toHi(old('end_time')) : ($defaultScheduleEnd ?? '22:00') }}"
                class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white" />
            @error('end_time')
                <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
            @enderror
        ...
        <div class="sm:col-span-2 lg:col-span-6 flex flex-wrap items-center gap-3">
            <button type="submit" form="quick-lesson-form"
