...
@php
    ...
    $base =
        'block w-full rounded-lg border bg-white dark:bg-gray-800 px-3 py-2.5 text-sm text-gray-900 dark:text-gray-100 placeholder-transparent transition focus:outline-none focus:ring-2 resize-none disabled:opacity-50';
...
<div class="relative">
    <textarea name="{{ $name }}" id="{{ $name }}" rows="{{ $rows }}" placeholder="{{ $label }}"
        @required($required) {{ $attributes->class([$cls]) }}>{{ old($name, $value) }}</textarea>
