...
@section('title', 'Entrar — Docente')

@section('content')
    <x-portal.auth-frame
        title="Entrar na área do docente"
        subtitle="Gerencie turmas, fichas de presença, aulas e quizzes da {{ $tenant->portalBrandTitle() }}."
        context-label="Área do docente"
        context-hint="Acesso exclusivo para professores cadastrados pela coordenação. Use o mesmo e-mail e senha do tenant."
    >
        @if (session('status'))
        ...
        <form method="POST" action="{{ route('tenant.login.store') }}" class="space-y-5">
            ...
            <div>
                <div class="flex items-center justify-between gap-3">
                    ...
                    <a href="{{ route('portal.acesso.docente.forgot') }}" class="shrink-0 text-xs font-semibold" style="color:var(--portal-primary)">Esqueci minha senha</a>
                </div>
        ...
        <p class="mt-8 border-t border-slate-100 pt-6 text-center text-sm text-slate-600 dark:border-slate-800 dark:text-slate-400">
            É estudante ou participante dos cursos?
            <a href="{{ route('portal.acesso.login') }}" class="font-semibold underline decoration-slate-300 underline-offset-2 hover:decoration-slate-500" style="color:var(--portal-primary)">
                Acesso ao aluno
            </a>
        </p>
    </x-portal.auth-frame>
